...
export default function JobShow() {
  ...
  const fetchMarkdownContent = useCallback(async () => {
    ...
    try {
      ...
      // Use the jobs proxy for markdown endpoint
      const response = await fetchWithAuth(
        `/api/jobs-proxy/${documentId}/markdown?includeReferences=true`
      );
  ...
  const getProcessingDocument = useCallback(async () => {
    ...
    try {
      ...
      // Using the jobs proxy and fields parameter
      const fields = '_id,title,status,createdAt,output';
      const response = await fetchWithAuth(
        `/api/jobs-proxy/${processingDocId}/processing?fields=${fields}`,
        {
          cache: 'no-store',
          headers: {
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      // Use jobs proxy for PDF download
      const response = await fetchWithAuth(
        `/api/jobs-proxy/${documentId}/pdf?includeReferences=true`,
        { method: 'GET' }
      );
  ...
  const { queryResult } = useShow<Job>({
    ...
    meta: {
      ...
      // Use the jobs proxy instead of direct API call
