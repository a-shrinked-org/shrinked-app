// app/api/usage-proxy/[subscriptionId]/[usageType]/route.ts
...
export async function GET(
  request: NextRequest,
  { params }: { params: { subscriptionId: string; usageType: string } }
) {
  const startTime = Date.now();
  const { subscriptionId, usageType } = params;
  
  if (IS_DEV) console.log(`[Usage Proxy] GET usage data for subscription: ${subscriptionId}, type: ${usageType}`);
  ...
  try {
	...
	// Direct API call 
	const apiUrl = `${API_URL}/usage/${subscriptionId}/${usageType}`;
