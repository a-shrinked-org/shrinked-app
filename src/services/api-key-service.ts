...
export const ApiKeyService = {
  async getApiKeys(): Promise<ApiKey[]> {
	try {
	  const response = await authUtils.fetchWithAuth(`${API_CONFIG.API_URL}/users/api-keys`);
  ...
  async deleteApiKey(keyId: string): Promise<boolean> {
	try {
	  const response = await authUtils.fetchWithAuth(`${API_CONFIG.API_URL}/users/api-key/${keyId}`, {
		method: "DELETE",
	  });
  ...
  async regenerateApiKey(keyId: string): Promise<ApiKey> {
	try {
	  const response = await authUtils.fetchWithAuth(`${API_CONFIG.API_URL}/users/api-key/${keyId}/regenerate`, {
		method: "POST",
	  });
